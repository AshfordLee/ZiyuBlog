...
在状态$$A_{i}$$执行动作$$B_{i}$$并且获得奖励$$R_{i}$$,其中i是轨迹的每一步的index

<img src="/images/RL_1/image8.png" alt="label_plot" />
...
<img src="/images/RL_1/image9.png" alt="label_plot" />

### 回报(Return)

回报指的是沿着一条轨迹从头到尾走完, 把每一步的Reward求和得到的最后数值, 一般来说用回报来衡量一个策略的好坏

有个数学问题是, 如果轨迹无限长, 有可能部分和是发散的, 为了确保回报收敛需要引入一个折价因子$$\gamma \in (0,1)$$, 例如
...
第一个条件概率度量在状态s执行动作a后变化到其他状态s'的概率$p(s'|s,a)$, 显然有以下概率之和的恒等式成立:

$$
\sum_{s' \in \mathcal{S}} p(s'|s,a) = 1
$$

第二个条件概率度量在状态s执行动作a后获得奖励r的概率$p(r|s,a)$, 显然有以下概率之和的恒等式成立:

$$
\sum_{r \in \mathcal{R}} p(r|s,a) = 1
$$

MDP还具备一种性质,即当前状态$$s_t$$只和前一个状态$$s_{t-1}$$以及前一个动作$$a_{t-1}$$有关, 而和更早的状态和动作无关, 即:
...
<img src="/images/RL_1/image10.png" alt="label_plot" />
